...
class FloatingBall:
    ...
    def show(self):
        ...
        self.canvas.pack()
        self.ring = self.canvas.create_oval(sp(4), sp(4), d - sp(4), d - sp(4),
                                            outline=app.accent, width=sp(4), fill=p["card"])
    ...
    def update(self, current=None):
        ...
        if self.canvas is not None and self.ring is not None:
            self.canvas.itemconfig(self.ring, outline=self.app.accent)

    def restyle(self):
        if self.canvas is None or self.ring is None:
            return
        p = self.app.palette
        self.canvas.configure(bg=p["magic"])
        self.canvas.itemconfig(self.ring, outline=self.app.accent, fill=p["card"])
        if self.time_label is not None:
